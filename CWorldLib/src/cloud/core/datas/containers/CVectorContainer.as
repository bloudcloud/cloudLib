package cloud.core.datas.containers
{
	 ...
	 * @author cloud
	 */
	public class CVectorContainer extends AbstractDataContainer implements ICPoolObject
	{
		private static var _Pool:ICObjectPool;
		
		private var _isInPool:Boolean;
		
		public function CVectorContainer()
		{
			initObject();
		}
...
//				_container=_container.concat(datas);
		...
		{
			var clone:CVectorContainer=CreateOneInstance();
			clone.add(this);
			return clone;
		}
		/**
		...
		public static function set IsUsePool(value:Boolean):void
		{
			if(value && _Pool==null)
			{
				CPoolsManager.Instance.registPool(CVectorContainer,10);
				_Pool=CPoolsManager.Instance.getPool(CVectorContainer);
			}
			else if(!value && _Pool!=null)
			{
				CPoolsManager.Instance.unRegistPool(CVectorContainer);
				_Pool=null;
			}
		 ...
		 * 
		 */		
		public static function CreateOneInstance():CVectorContainer
		{
			return _Pool==null ? new CVectorContainer() : _Pool.pop() as CVectorContainer;
		}
	}
}