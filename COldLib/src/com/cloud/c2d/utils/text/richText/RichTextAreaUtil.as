...
{
	import com.cloud.c2d.utils.text.HtmlUtil;
	import com.cloud.c2d.utils.text.StringUtils;
		...
		private static function htmlToXml(content:String,txtColor:String,size:int):XML
		{
			...
			for each (var str:String  in splitArr) 
			{
				...
				if(str.indexOf("<") != -1){
					...
					xml.appendChild(new XML(StringUtils.paramContent(fontTxt,txtColor,size,str)));
				}
			}
		...
		public static function getContentArr(content:String,txtColor:String="#d8e5c6",size:int=12):GroupElement
		{
			...
			for each (var xml:XML in toXml.children()) 
			{
				...
				if(tagName == IMG){
					...
					fmt.fontSize = size;
					fmt.color = color;
					fmt.fontDescription = bold?fontDescriptionBold:fontDescription;
