package utils.lx.managers
{
	...
	import utils.lx.managers.task.IRPCRequestTaskData;
	import utils.lx.managers.task.L3DRPCRequestTaskData;

	public class ServiceMGR
	{
		private static const _LIMIT_REQUESTNUM:int = 10;
		
		private var _sendQueue:Vector.<IRPCRequestTaskData>;
		private var _requestDataMap:CHashMap;
		private var _isRunning:Boolean;
		private var _timerCount:Number;
		private var _excuteNum:int;
		
		public function ServiceMGR()
		{
			_sendQueue=new Vector.<IRPCRequestTaskData>();
			_requestDataMap=new CHashMap();
		}
		...
		private function doRequest(taskData:IRPCRequestTaskData):void
		{
			_excuteNum++;
			_timerCount=getTimer();
			var op:Operation = webService[taskData.requestName];
			var atObj:AsyncToken = op.send.apply(null,taskData.requestParams);
			var rpObj:Responder = responder(onRequessTaskSuccessCallback, onRequessTaskFaultCallback);
			atObj.addResponder(rpObj);
			_requestDataMap.put(atObj,taskData);
			GlobalManager.log.print(LogFlagCustom.LoadResource,"发送请求！requestName: {0} requestParams: {1}",taskData.requestName,taskData.requestParams);
		}
		
		private function onRequessTaskSuccessCallback(reObj:ResultEvent):void
		{
			var taskData:L3DRPCRequestTaskData=_requestDataMap.get(reObj.token) as L3DRPCRequestTaskData;
			GlobalManager.log.print(LogFlagCustom.LoadResource,"{0}-> 返回请求成功！耗时：{1}，参数：{2}",taskData.requestName,(getTimer()-_timerCount),taskData.toString());
			_excuteNum--;
			...
			taskData.clear();
			_requestDataMap.remove(reObj.token);
			//异步调用队列不为空，继续执行异步调用
			if(_sendQueue.length>0)
			{
				doRequest(_sendQueue.shift());
			}
			else if(_requestDataMap.size==0)
			{
				_isRunning=false;
			}
		}
		
		private function onRequessTaskFaultCallback(feObj:FaultEvent):void
		{
			var taskData:L3DRPCRequestTaskData=_requestDataMap.get(feObj.token) as L3DRPCRequestTaskData;
			GlobalManager.log.print(LogFlagCustom.LoadResource,"{0}-> 返回第{1}次请求失败! 耗时：{2}",taskData.requestName,(taskData.count+1),(getTimer()-_timerCount));
			_excuteNum--;
			if(_sendQueue.length==0 && _requestDataMap.size==0)
			{
				_isRunning=false;
				return;
			}
			...
			taskData.clear();
			_requestDataMap.remove(feObj.token);
		}
		...
		{
			_sendQueue.push(new L3DRPCRequestTaskData(name,success,fault,params));
			return true;
		}
		/**
		 * 执行所有请求任务 
		...
		{
			GlobalManager.log.print(LogFlagCustom.LoadResource," 请求入队！待废弃流程！");
			if(!_isRunning && _sendQueue.length>0)
			{
				_isRunning=true;
				//异步调用队列不为空，继续执行异步调用
				doRequest(_sendQueue[0]);
			}
		}
		/**
		 * 发送Web请求
		 ...
		 * 
		 */		
		public function sendWebRequest(name:String,success:Function,fault:Function,...params):void
		{
			GlobalManager.log.print(LogFlagCustom.LoadResource,"{0}-> 请求入队！",name);
			_isRunning=true;
			
			var requestTaskData:L3DRPCRequestTaskData=new L3DRPCRequestTaskData(name,success,fault,params);
			if(_excuteNum<_LIMIT_REQUESTNUM)
			{
				doRequest(requestTaskData);
			}
			else
			{
				_sendQueue.push(requestTaskData);
			}
		}
		 ...
		 * @param brand
		 */		
		public function downloadRootNodeXMLByCompanyName(success:Function,fault:Function,brand:String):void
		{
			var atObj:AsyncToken = webService.DownloadRootNodeXMLByCompanyName(brand);
		 ...
		 * @param brand
		 */	
		public function downloadRootNodeJsonByCompanyName(success:Function,fault:Function,brand:String):void
		{
			var atObj:AsyncToken = webService.DownloadRootNodeJsonByCompanyName(brand);
			var rpObj:Responder = responder(success, fault);
			atObj.addResponder(rpObj);
		}

		public function lejiaLogin(success:Function,fault:Function, userName:String, passWord:String):void
		{
			var atObj:AsyncToken = webService.LeJiaLogin(userName,passWord);
			var rpObj:Responder = responder(success, fault);
			atObj.addResponder(rpObj);
		}
		
		public function lejiaLoginFromCustomer(success:Function,fault:Function,userName:String,passWord:String,company:String):void
		{
			var atObj:AsyncToken = webService.LeJiaLoginFromCustomer(userName,passWord,company);
			var rpObj:Responder = responder(success, fault);
			atObj.addResponder(rpObj);
		}
		/**
...
//				atObj.addResponder(rpObj);
				...
				var listener:ListenerExtend = new ListenerExtend();
				//zh 2018.8.3
				GlobalManager.Instance.serviceMGR.sendWebRequest(CWebServiceDict.REQUEST_GETVIEWDETAILBUFFERFROMCODE,listener.concat(loadCurtainProjectDataBufferSearchResult,success,mainClothCode,type), listener.concat(loadCurtainProjectDataBufferSearchFault,mainClothCode, type), mainClothCode, type);
		...
		private function loadCurtainProjectDataBufferSearchResult(reObj:ResultEvent,success:Function,code:String,type:int):void
		{
			...
			else
			{
				...
				var listener:ListenerExtend = new ListenerExtend();
				//zh 2018.8.3
				GlobalManager.Instance.serviceMGR.sendWebRequest(CWebServiceDict.REQUEST_GETUSERSPACEVIEWDETAILBUFFERFROMCODE,listener.concat(searchUserSpaceMaterialInformationResult,success),searchUserSpaceMaterialInformationFault, LivelyLibrary.sceneUserCode, code, type);
			}			
		}			
		...
		{
			var brandIndex:int = LivelyLibrary.CurtainBrandIndex;
		...
		public function uploadUserSpaceMaterial(success:Function, fault:Function, userCode:String, buffer:ByteArray, code:String, name:String, brand:String, spec:String, mode:int, family:String, nodeName:String, xml:String, origin:Boolean=false):void
		{
			var atObj:AsyncToken = webService.UploadUserSpaceMaterial(userCode, buffer, code, name, brand, spec, mode, family, nodeName, xml, true);
			var rpObj:Responder = responder(success, fault);
			atObj.addResponder(rpObj);
		}
