package utils.poly2tri {
	...
	public class Triangle {
		// Triangle points
		public var points:Vector.<PPoint> = new Vector.<PPoint>(3, true); // [null, null, null]
		...
		public function Triangle(p1:PPoint = null, p2:PPoint = null, p3:PPoint = null, fixOrientation:Boolean = false, checkOrientation:Boolean = true) 
		{
		...
		/*	if(EqualPoints(p1, p2, 0.1))
			{
				p1.x += 0.1;
				p1.y += 0.1;
			}
			else if(EqualPoints(p1, p3, 0.1))
			{
				p1.x += 0.1;
				p1.y += 0.1
			}
			else if(EqualPoints(p2, p3, 0.1))
			{
				p2.x += 0.1;
				p2.y += 0.1;
			} */
		...
		public function EqualPoints(v1:PPoint, v2:PPoint, tolerance:Number):Boolean
		{
		 ...
		 */
		public function containsPoint(point:PPoint):Boolean {
		...
		public function containsEdgePoints(p1:PPoint, p2:PPoint):Boolean {
			// In a triangle to check if contains and edge is enough to check if it contains the two vertices.
		 ...
		 */
		public function markNeighbor(t:Triangle, p1:PPoint, p2:PPoint):void {
		...
		// Optimized?
		public function getPointIndexOffset(p:PPoint, offset:int = 0):uint {
			var no:int = offset;
		 ...
		 * @return
		 */
		public function isPointAVertex(p:PPoint):Boolean {
		...
		static public function getNotCommonVertex(t1:Triangle, t2:Triangle):PPoint {
			return t1.points[getNotCommonVertexIndex(t1, t2)];
		}
		
		static public function getCommonEdge(t1:Triangle, t2:Triangle):Edge {
			var commonIndexes:Vector.<PPoint> = new Vector.<PPoint>();
			for each (var point:PPoint in t1.points) {
		...
		public function pointCW (p:PPoint):PPoint { return this.points[getPointIndexOffset(p, CCW_OFFSET)]; }
		public function pointCCW(p:PPoint):PPoint { return this.points[getPointIndexOffset(p, CW_OFFSET)]; }
		
		public function neighborCW(p:PPoint):Triangle { return this.neighbors[getPointIndexOffset(p, CW_OFFSET)]; }
		public function neighborCCW(p:PPoint):Triangle { return this.neighbors[getPointIndexOffset(p, CCW_OFFSET)]; }

		public function getConstrainedEdgeCW(p:PPoint):Boolean              { return this.constrained_edge[getPointIndexOffset(p, CW_OFFSET)]; }
		public function setConstrainedEdgeCW(p:PPoint, ce:Boolean):Boolean  { return this.constrained_edge[getPointIndexOffset(p, CW_OFFSET)] = ce; }

		public function getConstrainedEdgeCCW(p:PPoint):Boolean             { return this.constrained_edge[getPointIndexOffset(p, CCW_OFFSET)]; }
		public function setConstrainedEdgeCCW(p:PPoint, ce:Boolean):Boolean { return this.constrained_edge[getPointIndexOffset(p, CCW_OFFSET)] = ce; }

		public function getDelaunayEdgeCW(p:PPoint):Boolean                 { return this.delaunay_edge[getPointIndexOffset(p, CW_OFFSET)]; }
		public function setDelaunayEdgeCW(p:PPoint, e:Boolean):Boolean      { return this.delaunay_edge[getPointIndexOffset(p, CW_OFFSET)] = e; }
		
		public function getDelaunayEdgeCCW(p:PPoint):Boolean                { return this.delaunay_edge[getPointIndexOffset(p, CCW_OFFSET)]; }
		public function setDelaunayEdgeCCW(p:PPoint, e:Boolean):Boolean     { return this.delaunay_edge[getPointIndexOffset(p, CCW_OFFSET)] = e; }

		/**
		 * The neighbor across to given point.
		 */
		public function neighborAcross(p:PPoint):Triangle { return this.neighbors[getPointIndexOffset(p, 0)]; }
		public function oppositePoint(t:Triangle, p:PPoint):PPoint { return this.pointCW(t.pointCW(p)); }
		 ...
		 * parameter).
		 */
		public function legalize(opoint:PPoint, npoint:PPoint = null):void {
		 ...
		 * @param	p
		 */
		public function index(p:PPoint):int {
			try {
		...
		public function edgeIndex(p1:PPoint, p2:PPoint):int {
		...
		public function markConstrainedEdgeByPoints(p:PPoint, q:PPoint):void {
		 ...
		 * @return
		 */
		public function isEdgeSide(ep:PPoint, eq:PPoint):Boolean {
		 ...
		 * </pre>
		 */
		static public function rotateTrianglePair(t:Triangle, p:PPoint, ot:Triangle, op:PPoint):void {
		...
		static public function getUniquePointsFromTriangles(triangles:Vector.<Triangle>):Vector.<PPoint> {
		...
		static public function traceList(triangles:Vector.<Triangle>):void {
			var point:PPoint;
			var triangle:Triangle;
			var pointsList:Vector.<PPoint> = Triangle.getUniquePointsFromTriangles(triangles);
...
//			}
		...
		public function pointInsideTriangle(pp:PPoint):Boolean {
			var p1:PPoint = points[0];
			var p2:PPoint = points[1];
			var p3:PPoint = points[2];
		...
		static private function _product(p1:PPoint, p2:PPoint, p3:PPoint):Number {
