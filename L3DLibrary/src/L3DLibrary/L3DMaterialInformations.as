package L3DLibrary
{
	...
	public class L3DMaterialInformations extends EventDispatcher
	{
		...
		private var _meshBuffer:ByteArray;
		private var _textureBuffer:ByteArray;
		 ...
		 * 模型数据字节流 
		 */		
		public function get meshBuffer():ByteArray
		{
			return _meshBuffer;
		}
		public function set meshBuffer(value:ByteArray):void
		{
			_meshBuffer=value;
		}
		/**
		 * 纹理图片字节流 
		 */		
		public function get textureBuffer():ByteArray
		{
			return _textureBuffer;
		}
		public function set textureBuffer(value:ByteArray):void
		{
		 ...
		 * 
		 */		
		public function get textureBmp():BitmapData
		{
			return _textureBmp;
		}
		public function set textureBmp(value:BitmapData):void
		{
			_textureBmp = value;
		}
		...
		public function L3DMaterialInformations(buffer:ByteArray = null, url:String = null)
		{
			...
			{
				print("error:","L3DMaterialInformations",error);
			}
		}
		...
		/**水平平面光*/
		public static function BuildRectLightTBMaterialInformations(length:Number, width:Number, height:Number):L3DMaterialInformations
		{
			if(length < 5 || width < 5 || height < 5)
			{
				return null;
		...
		/**垂直平面光*/
		public static function BuildRectLightLRMaterialInformations(length:Number, width:Number, height:Number):L3DMaterialInformations
		{
			if(length < 5 || width < 5 || height < 5)
			{
				return null;
...
//		public static function BuildVerticalRectLight(length:Number, width:Number, height:Number):L3DMaterialInformations
//		{
//			if(length < 5 || width < 5 || height < 5)
//			{
//				return null;
...
//		public static function BuildRectLightVectrl
		...
		private function Load(buffer:ByteArray):Boolean
		{
			...
			GlobalManager.log.print(LogFlagCustom.LoadResource,"spec={0}",spec);
		...
		public function DownloadMaterial():void
		{
			...
			//zh 2018.8.3
			GlobalManager.Instance.serviceMGR.sendWebRequest(CWebServiceDict.REQUEST_DOWNLOADMATERIAL,LoadMaterialDataResult, LoadMaterialDataFault,url);
		}
		...
		public function DownloadMaterialData(code:String, url:String):void
		{
			...
			{
				//zh 2018.8.3
				GlobalManager.Instance.serviceMGR.sendWebRequest(CWebServiceDict.REQUEST_DOWNLOADMATERIAL,DownLoadMaterialDataResult, DownLoadMaterialDataFault,url);
			}
			else
			{ 
				//zh 2018.8.3
				GlobalManager.Instance.serviceMGR.sendWebRequest(CWebServiceDict.REQUEST_GETVIEWDETAILBUFFERFROMCODE,SearchMaterialDataResult, SearchMaterialDataFault, searchCode, 3);
		...
		private function SearchMaterialDataResult(reObj:ResultEvent):void
		{
			...
			if(buffer != null && buffer.length > 0)
			{
				...
				{
					//zh 2018.8.3
					GlobalManager.Instance.serviceMGR.sendWebRequest(CWebServiceDict.REQUEST_DOWNLOADMATERIAL,DownLoadMaterialDataResult, DownLoadMaterialDataFault,information.url);
				}
				else
				{
					var event1:L3DLibraryEvent = new L3DLibraryEvent(L3DLibraryEvent.DownloadMaterial);
		...
		public function DownloadMaterialLinkedData(code:String, url:String):void
		{
			...
			{
				//zh 2018.8.3
				GlobalManager.Instance.serviceMGR.sendWebRequest(CWebServiceDict.REQUEST_DOWNLOADMATERIALDETAILBUFFER, LoadMaterialLinkedDataResult, LoadMaterialLinkedDataFault, url, 3);
			}
			else
			{
				//zh 2018.8.3
				GlobalManager.Instance.serviceMGR.sendWebRequest(CWebServiceDict.REQUEST_GETVIEWDETAILBUFFERFROMCODE,SearchMaterialLinkedDataResult, SearchMaterialLinkedDataFault, searchCode, 3);
 			}
		}
		...
		private function LoadMaterialLinkedDataResult(reObj:ResultEvent):void
		{
			...
			//	information.DownloadLinkedDataBuffer();
				...
				{
					//cloud 2018.8.2
					GlobalManager.Instance.serviceMGR.sendWebRequest(CWebServiceDict.REQUEST_DOWNLOAD,DownloadLinkedDataResult,DownloadLinkedDataFault,information.linkedDataUrl);
				}
			}
		}
		...
		public function DownloadMaterialInformationData(code:String, url:String, type:int = 0):void
		{
			...
			{
				//zh 2018.8.3
				GlobalManager.Instance.serviceMGR.sendWebRequest(CWebServiceDict.REQUEST_DOWNLOADMATERIALDETAILBUFFER, LoadMaterialInformationDataResult, LoadMaterialInformationDataFault, url, searchType);
				 
			}
			else
			{
				//zh 2018.8.3
				GlobalManager.Instance.serviceMGR.sendWebRequest(CWebServiceDict.REQUEST_GETVIEWDETAILBUFFERFROMCODE,SearchMaterialInformationDataResult, SearchMaterialInformationDataFault, searchCode, searchType);
			}
		}
...
//			}
		 ...
		 * 
		 */		
		public function loadTextureBitmapdata():void
		{
//			if(_textureBmp)
//			{
//				var evt:L3DLibraryEvent = new L3DLibraryEvent(L3DLibraryEvent.LOAD_TEXTURE_BITMAPDATA);
//				evt.PreviewBitmap = _textureBmp;
//				this.dispatchEvent(evt);
//			}
			if(_textureBuffer == null || _textureBuffer.length == 0)
			{
				var evt:L3DLibraryEvent = new L3DLibraryEvent(L3DLibraryEvent.LOAD_TEXTURE_BITMAPDATA);
				evt.PreviewBitmap = new BitmapData(8,8,false,0xffffff);
				this.dispatchEvent(evt);
			...
			var loader:Loader = new Loader();
			loader.loadBytes(previewBuffer);
			loader.contentLoaderInfo.addEventListener(Event.COMPLETE, onLoadTextureBitmapdataSuccess);	
		}
		private function onLoadTextureBitmapdataSuccess(event:Event):void
		{
			event.currentTarget.removeEventListener(event.type,onLoadTextureBitmapdataSuccess);
		...
		{
			event.currentTarget.removeEventListener(event.type,loaderPreviewBitmapComplete);
			var loaderInfo:LoaderInfo = LoaderInfo(event.target);
			var checkBitmap:BitmapData=(loaderInfo.loader.content as Bitmap).bitmapData.clone();
//			var checkBitmap:BitmapData = new BitmapData(loaderInfo.width, loaderInfo.height, catalog == 21 || catalog == 28, 0xFFFFFF);
//			checkBitmap.draw(loaderInfo.loader);
		...
		public function DownloadTopview():void
		{
			...
			//zh 2018.8.3
			GlobalManager.Instance.serviceMGR.sendWebRequest(CWebServiceDict.REQUEST_DOWNLOADMATERIALTOPVIEWBUFFER,LoadTopviewDataResult, LoadTopviewDataFault, url);
		}
...
//				loader.contentLoaderInfo.addEventListener(Event.COMPLETE, loaderClipBitmapComplete);	
				...
				{
					print("error:","loaderCheckBitmapComplete",error);
				}
			}
		}
...
//			topviewClipBitmap.draw(loaderInfo.loader);
			...
			{
				print("error:","loaderClipBitmapComplete->",error);
				length = 0;
		...
		public function DownloadLinkedRoomXML():void
		{
			...
			//zh 2018.8.3
			GlobalManager.Instance.serviceMGR.sendWebRequest(CWebServiceDict.REQUEST_DOWNLOADUSERFILECONTENTSXML,DownloadLinkedRoomXMLResult, DownloadLinkedRoomXMLFault, linkedDataUrl);
		}
		...
		public function SearchMaterialInformation(code:String, type:int = 0):void
		{
			...
			searchType = type;
			//zh 2018.8.3
			GlobalManager.Instance.serviceMGR.sendWebRequest(CWebServiceDict.REQUEST_GETVIEWDETAILBUFFERFROMCODE,SearchMaterialInformationResult, SearchMaterialInformationFault, code, type);
		...
		private function SearchMaterialInformationResult(reObj:ResultEvent):void
		{
			...
			{
				var information:L3DMaterialInformations = new L3DMaterialInformations(buffer, "");
				information.seriesPath = ObjectUtils.clone(seriesPath);
				event.MaterialInformation = information;
				dispatchEvent(event);
			}
			else
			{
				//zh 2018.8.3
				GlobalManager.Instance.serviceMGR.sendWebRequest(CWebServiceDict.REQUEST_GETUSERSPACEVIEWDETAILBUFFERFROMCODE,SearchUserSpaceMaterialInformationResult, SearchUserSpaceMaterialInformationFault, LivelyLibrary.sceneUserCode, searchCode, searchType);
			}		
		}
		...
		public function Clone():L3DMaterialInformations
		{
			...
			if(_textureBuffer)
			{
				m.textureBuffer=new ByteArray();
				m.textureBuffer.writeBytes(_textureBuffer,0,_textureBuffer.length);
			}
			m.textureBmp=_textureBmp?_textureBmp.clone():null;
			m.xml=_xml;
			m.url = url;
		...
		public function Update():void
		{
			...
			if(!L3DLibrary.L3DLibraryWebService.WebServiceEnable)
			{
				return;
			}
			...
			//zh 2018.8.3
			GlobalManager.Instance.serviceMGR.sendWebRequest(CWebServiceDict.REQUEST_SEARCHMATERIALVIEWDETAILBUFFERPRO,UpdateMaterialViewDetailResult, UpdateMaterialViewDetailFault, code, 0, 3, 0, "");
		}
		...
		private function UpdateMaterialViewDetailResult(reObj:ResultEvent):void
		{
			...
			var material:L3DMaterialInformations;
			for(var i:int = 0; i<count; i++)
			{
				...
				if(length > 0)
				{
					...
					material = new L3DMaterialInformations(mBuffer);
...
//		private var searchType:int = 0;
		...
		public function Search(text:String, mode:int=0, type:int = 0, catalog:int = 0, pid:String = ""):void
		{
			...
			searchPid = pid;
			//zh 2018.8.3
			GlobalManager.Instance.serviceMGR.sendWebRequest(CWebServiceDict.REQUEST_SEARCHMATERIALVIEWDETAILBUFFERPRO,SearchMaterialViewDetailResult, SearchMaterialViewDetailFault, text, mode,type, catalog,pid);
		}
		
		public function SearchForRightMenu(text:String, mode:int=0, type:int = 0, catalog:int = 0, pid:String = ""):void
		{
			if(text == null || text.length == 0 || mode < 0)
			{
				return;
			...
			GlobalManager.Instance.serviceMGR.sendWebRequest(CWebServiceDict.REQUEST_RIGHTMRNUSEARCHMATERIALVIEWDETAILBUFFERPRO,SearchMaterialViewDetailResult, SearchMaterialViewDetailFault, text, mode,type, catalog,pid);
		}
		...
		private function SearchMaterialViewDetailResult(reObj:ResultEvent):void
		{
			...
			if(buffer == null || buffer.length == 0)
			{
				...
				{
					//zh 2018.8.3
					GlobalManager.Instance.serviceMGR.sendWebRequest(CWebServiceDict.REQUEST_SEARCHUSERSPACEMATERIALVIEWDETAILBUFFER,SearchUserSpaceMaterialViewDetailResult, SearchUserSpaceMaterialViewDetailFault, LivelyLibrary.sceneUserCode, searchText, searchType);
				}
				else
				{
				    userData = null;
		...
		private static function LoadBitmapData(buffer:ByteArray):BitmapData
		{
			...
			catch(error:Error)
			{
				bmpData = null;
				decoder = null;
				print("error:","LoadBitmapData->",error);
			}
