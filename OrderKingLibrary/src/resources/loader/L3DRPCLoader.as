package resources.loader
{
	...
	public class L3DRPCLoader extends EventDispatcher implements IL3DLoader
	{
		...
		private function onLoadMaterialInformationResultByUrl(evt:ResultEvent):void
		{
			...
			_curMaterialInfo = new L3DMaterialInformations(buffer, _curLoadTaskData.url);
		...
		private function onLoadPreviewResult(evt:L3DLibraryEvent):void
		{
			...
			if(_curMaterialInfo.url && _curMaterialInfo.url.length && _curMaterialInfo.url.lastIndexOf(".L3D")>=0)
			{
				...
				_curLoadTaskData._loadCount++;
				//lrj 2017/12/8 
				GlobalManager.Instance.serviceMGR.downloadMaterial(onLoadMeshBufferResult, onLoadMeshBufferFault, _curMaterialInfo.url);
			}
			else if(_curMaterialInfo.linkedDataUrl && _curMaterialInfo.linkedDataUrl.length)
			{
				...
				_curLoadTaskData._loadCount++;
				//lrj 2017/12/7 
				GlobalManager.Instance.serviceMGR.download(onLoadLinkedDataResult, onLoadLinkedDataFault, _curMaterialInfo.linkedDataUrl);
			}
			doLoadMaterialInfoComplete();
		}
		private function onSearchMaterialInforDataResult(evt:ResultEvent):void
		{
			...
			_curMaterialInfo = new L3DMaterialInformations(buffer,"");
			GlobalManager.Instance.resourceMGR.addInformationCode(_curLoadTaskData.code,_curMaterialInfo);
			_curMaterialInfo.addEventListener(L3DLibraryEvent.LoadPreview,onLoadPreviewResult);
			_curLoadTaskData._loadCount++;
			_curMaterialInfo.LoadPreview();
		}
		...
		protected function doLoadByUrl(url:String,rpcType:int=0):Boolean
		{
			...
			_curLoadTaskData._loadCount++;
			// 2017/12/8 lrj--
			GlobalManager.Instance.serviceMGR.downloadMaterialDetailBuffer(onLoadMaterialInformationResultByUrl, onLoadMaterialInformationFault, url, rpcType);
			return true;
		}
		/**
		 * 通过材质code，执行下载操作 
		...
		protected function doLoadByCode(code:String,rpcType:int=0):Boolean
		{
			...
			_curLoadTaskData._loadCount++;
			//lrj 2017/12/8 
			GlobalManager.Instance.serviceMGR.getViewDetailBufferFromCode(onSearchMaterialInforDataResult, onSearchMaterialInfoDataFault, code, rpcType);
			return true;
		}
		/**
		 * 通过nodeID，执行下载操作 
